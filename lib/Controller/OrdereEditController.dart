  ...
  factory Category.fromJson(Map<String, dynamic> json) {
    return Category(
      ...
      description: json['description'] ??
          'xyz', // Provide default value if description is missing or null
...
class Cart {
  ...
  void _saveCartToPreferences() async {
    ...
    prefs.setString('cart-edit', cartJson);
  ...
  Future<void> _loadCartFromPreferences() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? cartJson = prefs.getString('cart-edit');
    if (cartJson != null) {
    ...
    print('cart show $items');
  }

  void clearCartPreferences() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.remove('cart-edit');
    items.clear();
    print('Cart preferences cleared');
  }
}
...
class OrderEditController extends GetxController {
  final OrderTableModal? orderdata;
  var isLoading = false.obs;

  OrderEditController({required this.orderdata});
  ...
  void onInit() {
    ...
    if (orderdata != null && orderdata!.jsonData != null) {
      parseOrderData(orderdata!.jsonData!);
    }
  }

  void parseOrderData(String jsonData) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.remove('cart-edit');
    Map<String, dynamic> orderData = jsonDecode(jsonData);
    if (kDebugMode) {
      print("hi");
    }
    if (orderData.containsKey('sell_lines')) {
      List<dynamic> sellLines = orderData['sell_lines'];
      ...
      for (var item in sellLines) {
        ...
        Product obj = Product(
            ...
            price: double.tryParse(item['unit_price_inc_tax'])??0.00,
            categoryId: product['category_id']);
        int quantity = item['quantity'];
        cart.value.addToCart(obj, quantity);
      }

      cart.refresh();
      print('Order data added to cart: ${cart.value.items}');
    } else {
      print('sell_lines not found in order data');
    }
  }
  ...
  Future<void> updateOrder(OrderTableModal order) async {
